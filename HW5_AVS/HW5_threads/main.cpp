...
// Объявление функций программистов
void first_programmer_do_task();
void second_programmer_do_task();
void third_programmer_do_task();
void first_programmer_check_task();
void second_programmer_check_task();
void third_programmer_check_task();

// Функция, моделирующая работу первого программиста
void first_programmer(){
    while(all_tasks > 0) {
        first_programmer_do_task();
        first_programmer_check_task();
...
void second_programmer(){
    while(all_tasks > 0) {
        second_programmer_do_task();
        second_programmer_check_task();
        this_thread::sleep_for(chrono::microseconds(500));
    }
}
...
void third_programmer(){
    while(all_tasks > 0) {
        third_programmer_do_task();
        third_programmer_check_task();
        this_thread::sleep_for(chrono::microseconds(500));
    }
}

// Написание программы первым программистом
void first_programmer_do_task(){
    mtx.lock();
...
// Написание программы вторым программистом
void second_programmer_do_task(){
    mtx.lock();
    // Проверяем, нужно программисту писать программу и может ли он это сделать
...
// Написание программы третьим программистом
void third_programmer_do_task(){
    mtx.lock();
    // Проверяем, нужно программисту писать программу и может ли он это сделать
...
// Функция для проверки первым программистом присланной ему программы
void first_programmer_check_task(){
    mtx.lock();
...
// Функция для проверки вторым программистом присланной ему программы
void second_programmer_check_task(){
    mtx.lock();
...
// Функция для проверки третьим программистом присланной ему программы
void third_programmer_check_task(){
    mtx.lock();
